    ...
    if (file.is_open()){
        ...
        while (getline(file, line))
        {
            ...
            if (is_sorted(line_int.begin(), line_int.end(), comp_asc) || is_sorted(line_int.begin(), line_int.end(), comp_desc)){
                ...
                // If report is safe
                if (cant_asc_desc==line_int.size()-1) {
                    result++;
                }
                else{       
                    for (int i = 0; i < line_int.size(); i++){
                        vector<int> tmp=line_int;
                        tmp.erase(find(tmp.begin(), tmp.end(), line_int[i]));
                        if (is_sorted(tmp.begin(), tmp.end(), comp_asc) || is_sorted(tmp.begin(), tmp.end(), comp_desc)){
                            cant_asc_desc=0;
                            for (int j = 0; j < tmp.size()-1; j++){
                                if(abs(tmp[j+1]-tmp[j])>=1 && abs(tmp[j+1]-tmp[j])<=3)cant_asc_desc++;
                            }
                            if (cant_asc_desc==tmp.size()-1) {
                                result++;
                            }
                        }
                    }
